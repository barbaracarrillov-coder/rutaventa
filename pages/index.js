...
function MainApp({user,onLogout}){
  ...
  return(<><Head><title>RutaVenta</title><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"/><meta name="theme-color" content="#ea580c"/><link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>🗺</text></svg>"/></Head>
    ...
    <div style={{minHeight:"100vh",background:"#f8f9fb",fontFamily:"'Nunito',sans-serif",color:"#1e293b",display:"flex",flexDirection:"column"}}>
      ...
      {tab==="prospects"&&<div style={{background:"linear-gradient(135deg,#fff7ed,#fef3c7)",padding:"10px 16px",borderBottom:"2px solid #fed7aa",display:"flex",alignItems:"center",gap:"8px"}}><span style={{fontSize:"20px"}}>🎯</span><div><div style={{fontSize:"13px",fontWeight:800,color:"#92400e"}}>Meta de hoy</div><div style={{fontSize:"12px",color:"#a16207"}}>{clients.filter(c=>c.status==="nuevo").length>3?`Contacta ${Math.min(3,clients.filter(c=>c.status==="nuevo").length)} prospectos y sigue a ${Math.min(2,clients.filter(c=>c.status==="contactado").length)} contactados`:`Agrega ${Math.max(0,5-clients.length)} prospectos a tu cartera`}</div></div></div>}
      ...
      <main style={{flex:1,overflow:"auto",padding:"14px",maxWidth:"600px",margin:"0 auto",width:"100%"}}>
        ...
        {tab==="clients"&&<CL clients={clients} setStatus={setStatus} addNote={addNote} delClient={delClient} toRoute={toRoute} route={route} addClient={addClient}/>}
...
function Prospects({clients,addClient}){const[q,setQ]=useState("");const[loc,setLoc]=useState("");const[results,setResults]=useState([]);const[ld,setLd]=useState(false);const[searched,setSearched]=useState(false);
  const search=async c=>{const sq=c||q;if(!sq.trim()||!loc.trim())return;setLd(true);setSearched(true);try{const t=await callAI(`Generate 6-8 realistic Chilean businesses. ONLY JSON array. Address: "Calle Número, Ciudad" NO comuna NO región. Phone: "+56 9 XXXX XXXX". Each: name,address,type,phone,rating(1-5),id("p_"+random8).`,[{role:"user",content:`"${sq}" cerca de ${loc}, Chile`}],2000);setResults(JSON.parse(t.replace(/```json|```/g,"").trim()));}catch{setResults([]);}setLd(false);};
  const has=id=>clients.some(c=>c.id===id);
  return(<div style={{animation:"fadeIn 0.4s"}}><h2 style={{fontSize:"22px",fontWeight:900,marginBottom:"4px"}}>Buscar Prospectos</h2><p style={{fontSize:"14px",color:"#64748b",marginBottom:"14px"}}>Encuentra negocios cercanos</p>
    <input value={loc} onChange={e=>setLoc(e.target.value)} placeholder="📍 Tu ciudad (ej: Puerto Montt)" style={{...S.input,marginBottom:"10px"}} onFocus={e=>e.target.style.borderColor="#ea580c"} onBlur={e=>e.target.style.borderColor="#e2e8f0"}/>
    ...
    {ld&&<div style={{textAlign:"center",padding:"36px",color:"#94a3b8"}}><div style={{fontSize:"32px",marginBottom:"8px"}}>🔍</div>Buscando...</div>}
    {!ld&&searched&&results.length===0&&<div style={{textAlign:"center",padding:"36px",color:"#94a3b8"}}>Sin resultados.</div>}
    {results.map((r,i)=><div key={r.id||i} style={{...S.card,display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"10px",animation:`fadeIn 0.3s ease ${i*0.05}s both`}}><div style={{flex:1}}><div style={{fontSize:"16px",fontWeight:800,marginBottom:"2px"}}>{r.name}</div><div style={{fontSize:"13px",color:"#64748b",marginBottom:"4px"}}>{r.address}</div><div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}><span style={{fontSize:"12px",padding:"2px 8px",borderRadius:"10px",background:"#fff7ed",color:"#c2410c",fontWeight:700}}>{r.type}</span>{r.phone&&<span style={{fontSize:"12px",color:"#64748b"}}>📞 {r.phone}</span>}{r.rating&&<span style={{fontSize:"12px",color:"#d97706",fontWeight:700}}>{"★".repeat(Math.round(r.rating))}</span>}</div></div>{!has(r.id)?<button onClick={()=>addClient(r)} style={{padding:"9px 13px",background:"linear-gradient(135deg,#ea580c,#dc2626)",border:"none",borderRadius:"10px",color:"#fff",fontWeight:800,fontSize:"13px",cursor:"pointer",flexShrink:0}}>+ Agregar</button>:<span style={{padding:"9px 13px",background:"#d1fae5",borderRadius:"10px",color:"#059669",fontWeight:700,fontSize:"13px"}}>✓</span>}</div>)}
  </div>);
}

// ━━━ CLIENTS ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function CL({clients,setStatus,addNote,delClient,toRoute,route,addClient}){const[filter,setFilter]=useState("todos");const[openId,setOpenId]=useState(null);const[note,setNote]=useState("");
  const[showAdd,setShowAdd]=useState(false);const[newName,setNewName]=useState("");const[newAddr,setNewAddr]=useState("");const[newPhone,setNewPhone]=useState("");const[newType,setNewType]=useState("");
  const handleAddManual=()=>{if(!newName.trim())return;addClient({id:"m_"+Date.now()+"_"+Math.random().toString(36).slice(2,8),name:newName.trim(),address:newAddr.trim()||"Sin dirección",phone:newPhone.trim(),type:newType.trim()||"Cliente directo",rating:0});setNewName("");setNewAddr("");setNewPhone("");setNewType("");setShowAdd(false);};
  ...
  return(<div style={{animation:"fadeIn 0.4s"}}><h2 style={{fontSize:"22px",fontWeight:900,marginBottom:"4px"}}>Mi Cartera</h2><p style={{fontSize:"14px",color:"#64748b",marginBottom:"14px"}}>{clients.length} prospectos · {cnt.cliente||0} clientes</p>
    ...
    {showAdd&&<div style={{...S.card,border:"2px solid #10b981",marginBottom:"14px"}}>
      ...
      <input value={newType} onChange={e=>setNewType(e.target.value)} placeholder="Tipo (ej: Restaurante, Carnicería, Hotel)" style={{...S.input,marginBottom:"10px",fontSize:"15px"}} onFocus={e=>e.target.style.borderColor="#059669"} onBlur={e=>e.target.style.borderColor="#e2e8f0"}/>
    ...
    {list.map((c,i)=>{const cfg=STATUS[c.status];const open=openId===c.id;const inR=route.includes(c.id);return(<div key={c.id} style={{...S.card,padding:0,overflow:"hidden",animation:`fadeIn 0.3s ease ${i*0.03}s both`}}>
      <div onClick={()=>setOpenId(open?null:c.id)} style={{padding:"14px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"2px",flexWrap:"wrap"}}><span style={{fontSize:"15px",fontWeight:800}}>{c.name}</span><span style={{fontSize:"11px",padding:"2px 7px",borderRadius:"8px",background:cfg.bg,color:cfg.color,fontWeight:700}}>{cfg.icon}{cfg.label}</span>{inR&&<span style={{fontSize:"10px",padding:"2px 5px",borderRadius:"6px",background:"#fff7ed",color:"#ea580c",fontWeight:700}}>🗺️</span>}</div><div style={{fontSize:"12px",color:"#64748b"}}>{c.address}</div></div><span style={{color:"#94a3b8",transform:open?"rotate(180deg)":"none",transition:"0.2s"}}>▾</span></div>
      {open&&<div style={{padding:"0 14px 14px",borderTop:"2px solid #f1f5f9"}}><div style={{...S.section,marginTop:"10px"}}>Estado</div><div style={{display:"flex",gap:"4px",flexWrap:"wrap",marginBottom:"10px"}}>{Object.entries(STATUS).map(([k,v])=><button key={k} onClick={()=>setStatus(c.id,k)} style={{padding:"6px 10px",borderRadius:"8px",fontSize:"12px",fontWeight:700,cursor:"pointer",background:c.status===k?v.color:"#f8fafc",color:c.status===k?"#fff":v.color,border:`2px solid ${c.status===k?v.color:"#e2e8f0"}`}}>{v.icon}{v.label}</button>)}</div>
        <div style={{display:"flex",gap:"6px",marginBottom:"10px"}}>{!inR&&<button onClick={()=>toRoute(c.id)} style={{...S.btnSm,color:"#ea580c",borderColor:"#fed7aa",background:"#fff7ed",fontSize:"12px"}}>🗺️ A ruta</button>}<button onClick={()=>delClient(c.id)} style={{...S.btnSm,color:"#dc2626",borderColor:"#fecaca",background:"#fef2f2",fontSize:"12px"}}>🗑️ Eliminar</button></div>
...
function AG({clients,routeClients,priceText,streak,userName,todaysActions}){const fn=userName.split(" ")[0];
  ...
  const stats=`📊 Cartera: ${clients.length} prospectos, ${clients.filter(c=>c.status==="cliente").length} clientes, ${clients.filter(c=>c.status==="nuevo").length} sin contactar.\n${todaysActions>0?`✅ Hoy: ${todaysActions} acciones.`:"🎯 Aún no haces contactos hoy."}\n\nTe ayudo con: 🎯Pitch · 💬Objeciones · 💰Cotización · 📝WhatsApp · 📊Cartera\n\n¿Qué necesitas?`;
  ...
  const QA=[{label:"🎯 Pitch",m:"Prepárame un pitch corto para un prospecto nuevo."},{label:"💬 Caro",m:"Me dicen caro. Dame el guión para responder."},{label:"💰 Cotización",m:"Arma cotización con mis precios para un restaurante."},{label:"📝 WhatsApp",m:"Mensaje de seguimiento para un prospecto."},{label:"📊 Cartera",m:`Analiza: ${clients.length} prospectos, ${clients.filter(c=>c.status==="cliente").length} clientes, ${clients.filter(c=>c.status==="nuevo").length} nuevos. ¿Qué hago?`},{label:"🔥 Motívame",m:"Estoy desmotivado. Necesito un empujón."}];
  const send=async c=>{const text=c||input;if(!text.trim()||busy)return;setInput("");const um={role:"user",content:text};setMsgs(p=>[...p,um]);setBusy(true);
    try{const ctx=`VENDEDOR: ${userName}, racha ${streak.count}d, ${clients.length} prospectos, ${clients.filter(c=>c.status==="cliente").length} clientes, ${clients.filter(c=>c.status==="nuevo").length} nuevos, ${routeClients.length} paradas hoy.\n${priceText?`PRECIOS:\n${priceText}`:"(Sin precios)"}`;
